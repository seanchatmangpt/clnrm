        ...
        match self {
            ...
            Commands::Template { template, name, output } => {
                if let Some(out) = output {
                    // Just basic routing for now, if custom templates need output they should handle it internally.
                    // To stay simple:
            ...
            Commands::Report { input, output, format: _ } => {
            ...
            Commands::Dev { paths, debounce_ms, clear, only, timebox } => {
                let paths_ref = paths.unwrap_or_default();
                run_dev_mode_with_filters(&paths_ref, debounce_ms, clear, only, timebox).await
            }
            ...
            Commands::Diff { baseline, current, format: _, only_changes } => {
                diff_traces(&baseline, &current, only_changes)?;
                Ok(())
            }
            ...
            Commands::Graph { trace, format: _, highlight_missing, filter } => {
                visualize_graph(&trace, highlight_missing, filter)?;
                Ok(())
            }
            Commands::Repro { baseline, verify_digest, output } => {
                reproduce_baseline(&baseline, verify_digest, output).await
            }
            Commands::RedGreen { paths, expect, verify_red, verify_green } => {
                run_red_green_validation(&paths, expect, verify_red, verify_green).await
            }
            Commands::Render { template, map, output, show_vars } => {
                render_template_with_vars(&template, &map, output.as_deref(), show_vars)?;
                Ok(())
            }
            Commands::Spans { trace, grep, format: _, show_attrs, show_events } => {
                filter_spans(&trace, grep.as_deref(), show_attrs, show_events)?;
                Ok(())
            }
...
old_run_pattern = r"    pub async fn run\(self, _verbose: bool\) -> crate::error::Result<\(\)> \{\s*// CLI-GALL-1 Refusal: Command dispatch must be fully implemented to route subcommands\s*// For now, return an error to indicate this needs to be implemented\s*Err\(crate::error::CleanroomError::internal_error\(\s*\"CLI-GALL-1 Refusal: Command dispatch not yet implemented for new modular structure\"\s*\)\)\s*\}"
