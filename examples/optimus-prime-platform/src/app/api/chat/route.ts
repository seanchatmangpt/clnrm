...
import { ChatRequest, ChatResponse, detectVirtue, REWARD_URLS, PREMIUM_CTA_VARIANTS } from '@/lib/types';
...
async function handleChildChat(userInput: string, headers: Headers): Promise<Response> {
  ...
  responseHeaders.set('X-Reward-Url', REWARD_URLS[virtue as keyof typeof REWARD_URLS] || REWARD_URLS.courage);
  ...
  return new Response(response.toDataStreamResponse().body, {
...
- Premium CTR A: ${metrics.ab.A.views > 0 ? ((metrics.ab.A.clicks / metrics.ab.A.views) * 100).toFixed(1) : '0.0'}%
- Premium CTR B: ${metrics.ab.B.views > 0 ? ((metrics.ab.B.clicks / metrics.ab.B.views) * 100).toFixed(1) : '0.0'}%
