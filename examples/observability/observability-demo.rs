...
async fn test_observability_framework_self_test() -> Result<()> {
    ...
    let container = env.get_or_create_container("observability-test", || async {
        tokio::time::sleep(Duration::from_millis(50)).await;
        Ok::<String, clnrm_core::CleanroomError>("traced-container".to_string())
    }).await?;

    // Execute commands that should generate traces
    let _result1 = env.execute_in_container(&container, ["echo", "trace-test-1"]).await?;
    let _result2 = env.execute_in_container(&container, ["echo", "trace-test-2"]).await?;
    ...
    for i in 0..5 {
        let _container = env.get_or_create_container(&format!("metrics-test-{}", i), || async {
    ...
    let container_metrics = metrics.iter().filter(|m| m.name.contains("container")).count();
    let execution_metrics = metrics.iter().filter(|m| m.name.contains("execution")).count();

    println!("   Container metrics: {}", container_metrics);
    println!("   Execution metrics: {}", execution_metrics);

    assert!(container_metrics > 0, "Should collect container metrics");
    assert!(execution_metrics > 0, "Should collect execution metrics");
    ...
    let _simple_container = simple_env.get_or_create_container("zero-config-test", || async {
        Ok::<String, clnrm_core::CleanroomError>("zero-config-container".to_string())
    }).await?;

    let _simple_result = simple_env.execute_in_container(&_simple_container, ["echo", "zero-config-test"]).await?;
    ...
    println!("✅ Zero-config metrics: {}", simple_metrics.len());

    // Should have some observability data even with zero config
    assert!(simple_traces.len() >= 0, "Should have trace data");
    assert!(simple_metrics.len() >= 0, "Should have metrics data");

    let total_time = start_time.elapsed();
    println!("\n🎉 SUCCESS: Observability demo completed in {:?}", total_time);
...
async fn test_comprehensive_observability() -> Result<()> {
    ...
    let containers = futures::future::try_join_all(
        (0..3).map(|i| {
            let env = &env;
            async move {
                let container = env.get_or_create_container(&format!("perf-test-{}", i), || async {
                    tokio::time::sleep(Duration::from_millis(30)).await;
                    Ok::<String, clnrm_core::CleanroomError>(format!("perf-container-{}", i))
                }).await?;

                let result = env.execute_in_container(&container, ["sh", "-c", "echo 'perf work' && sleep 0.05"]).await?;
    ...
    let performance_traces = traces.iter().filter(|t| t.operation_name.contains("perf")).count();
    println!("   Performance-related traces: {}", performance_traces);

    assert!(performance_traces > 0, "Should capture performance scenario traces");
