...
"#;
    ...
    for scenario in &config.scenarios {
        ...
        let mut scenario_builder = clnrm_core::scenario(&scenario.name);
        
        for step in &scenario.steps {
            scenario_builder = scenario_builder.step(step.name.clone(), step.cmd.clone());
        }
        
        let result = scenario_builder.run_async().await?;
        println!("✅ Scenario '{}' completed in {}ms", scenario.name, result.duration_ms);
    }
