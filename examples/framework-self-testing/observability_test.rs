...
use clnrm_core::{
    ...
    CleanroomEnvironment, CleanroomError,
};
...
async fn main() -> Result<(), CleanroomError> {
    ...
    println!(
        "⏱️  Telemetry initialization completed in: {}ms",
    ...
    println!(
        "⏱️  Tracing test completed in: {}ms",
        tracing_duration.as_millis()
    );
    println!();
    ...
    for i in 0..3 {
        let _container = env
            .get_or_create_container(&format!("observability-test-{}", i), || {
    ...
    println!(
        "   ✅ Framework metrics: {} created, {} reused",
        created, reused
    );

    let integration_duration = start.elapsed();
    println!(
        "⏱️  Framework integration test completed in: {}ms",
        integration_duration.as_millis()
    );
    println!();
    ...
    println!(
        "⏱️  Logging test completed in: {}ms",
        logging_duration.as_millis()
    );
    println!();
