...
async fn main() -> Result<(), CleanroomError> {
    ...
    println!(
        "⏱️  Isolation test completed in: {}ms",
        isolation_duration.as_millis()
    );
    println!();
    ...
    assert_eq!(
        container1, "env1-container",
        "Environment 1 should have its own container"
    );
    assert_eq!(
        container2, "env2-container",
        "Environment 2 should have its own container"
    );
    assert_eq!(
        container3, "env3-container",
        "Environment 3 should have its own container"
    );
    ...
    println!(
        "⏱️  Registry isolation test completed in: {}ms",
        registry_duration.as_millis()
    );
    println!();
    ...
    println!(
        "   ✅ Environment 1 metrics: {} created, {} reused",
        created1, reused1
    );
    println!(
        "   ✅ Environment 2 metrics: {} created, {} reused",
        created2, reused2
    );
    println!(
        "   ✅ Environment 3 metrics: {} created, {} reused",
        created3, reused3
    );
    ...
    println!(
        "⏱️  Metrics isolation test completed in: {}ms",
    ...
    let handles = vec![
        tokio::spawn(async move {
            ...
            let container = env
                .get_or_create_container("concurrent-test", || {
    ...
    for (i, result) in results.iter().enumerate() {
        match result {
            Ok(Ok((session, container))) => {
                println!(
                    "   ✅ Concurrent environment {}: session={}, container={}",
                    i + 1,
                    session,
                    container
                );
    ...
    println!(
        "⏱️  Concurrent isolation test completed in: {}ms",
    ...
    let total_duration =
        isolation_duration + registry_duration + metrics_duration + concurrent_duration;
