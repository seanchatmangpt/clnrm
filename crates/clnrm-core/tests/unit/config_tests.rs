...
fn test_test_config_without_metadata_section_fails_get_name() {
    ...
    assert!(result.is_err());
    let error = result.unwrap_err();
    assert!(error.message.contains("must have either [meta] or [test.metadata]"));
}
...
fn test_volume_config_with_empty_container_path_fails_validation() {
    ...
    assert!(result.is_err());
    let error = result.unwrap_err();
    assert!(error.message.contains("Volume container path cannot be empty"));
}
...
fn test_volume_config_with_relative_container_path_fails_validation() {
    ...
    assert!(result.is_err());
    let error = result.unwrap_err();
    assert!(error.message.contains("Volume container path must be absolute"));
}
...
fn test_policy_config_with_cpu_usage_above_one_fails_validation() {
    ...
    assert!(result.is_err());
    let error = result.unwrap_err();
    assert!(error.message.contains("Max CPU usage must be between 0.0 and 1.0"));
}
...
fn test_cleanroom_config_with_zero_max_containers_fails_validation() {
    ...
    assert!(result.is_err());
    let error = result.unwrap_err();
    assert!(error.message.contains("Max containers must be greater than 0"));
}
