...
async fn test_readme_core_claims() -> Result<()> {
    ...
    assert!(test_results.total_tests > 0, "Framework should have self-tests");
    assert!(test_results.passed_tests > 0, "Framework self-tests should pass");
    
    // Claim: "Hermetic Integration Testing Framework"
    let env = CleanroomEnvironment::new().await?;
    assert!(!env.session_id().is_nil(), "Each test should have unique session ID for isolation");
...
async fn test_readme_container_reuse_claims() -> Result<()> {
    ...
    let container1 = env.get_or_create_container("test_container", || {
    ...
    let container2 = env.get_or_create_container("test_container", || {
...
async fn test_readme_observability_claims() -> Result<()> {
    ...
    let _result = env.execute_test("observability_test", || {
...
fn test_readme_rich_assertions_claims() -> Result<()> {
    ...
    assert!(retrieved_value.is_some(), "Assertion context should store and retrieve data");
...
fn test_readme_policy_system_claims() -> Result<()> {
    ...
    assert!(env_vars.contains_key("CLEANROOM_SECURITY_LEVEL"), "Policy should generate environment variables");
    assert!(env_vars.contains_key("CLEANROOM_NETWORK_ISOLATION"), "Policy should control network isolation");
    ...
    assert!(operation_allowed, "Valid operations should be allowed by policy");
...
async fn test_readme_performance_claims() -> Result<()> {
    ...
    let container1 = env.get_or_create_container("perf_test", || {
        Ok::<String, CleanroomError>("performance_test_container".to_string())
    }).await?;
    
    let container2 = env.get_or_create_container("perf_test", || {
...
fn test_readme_error_handling_claims() -> Result<()> {
    ...
    assert!(error_string.contains("Test error"), "Error should display message");
    assert!(error_string.contains("Test context"), "Error should display context");
    assert!(error_string.contains("Test source"), "Error should display source");
...
async fn test_readme_eat_your_own_dog_food_claims() -> Result<()> {
    ...
    assert!(test_results.total_tests >= 5, "Framework should have comprehensive self-tests");
    assert!(test_results.passed_tests > 0, "Framework self-tests should pass");
    
    // Verify test results structure
    assert!(!test_results.test_results.is_empty(), "Should have individual test results");
    assert!(test_results.total_duration_ms > 0, "Should record execution time");
...
async fn test_readme_architecture_claims() -> Result<()> {
    ...
    assert!(!env.session_id().is_nil(), "Environment should have session ID");
    
    // Test backend access
    let backend = env.backend();
    assert_eq!(backend.name(), "testcontainers", "Environment should have testcontainers backend");
    assert!(backend.is_available(), "Backend should be available");
    assert!(backend.supports_hermetic(), "Backend should support hermetic execution");
    assert!(backend.supports_deterministic(), "Backend should support deterministic execution");
    
    // Test service registry access
    let services = env.services().await;
    assert!(services.active_services().is_empty(), "New environment should have no active services");
...
async fn test_readme_observability_detailed_claims() -> Result<()> {
    ...
    for i in 0..3 {
        let _result = env.execute_test(&format!("metric_test_{}", i), || {
...
"#;
    ...
    assert!(parsed.get("services").is_some(), "Should parse services section");
    assert!(parsed.get("steps").is_some(), "Should parse steps array");
    
    let test_section = parsed.get("test").unwrap();
    assert_eq!(test_section.get("name").unwrap().as_str(), Some("container_lifecycle_test"));
    assert_eq!(test_section.get("description").unwrap().as_str(), Some("Test that containers start, execute commands, and cleanup properly"));
    
    let steps = parsed.get("steps").unwrap().as_array().unwrap();
    assert_eq!(steps.len(), 1, "Should parse steps array");
    
    let step = &steps[0];
    assert_eq!(step.get("name").unwrap().as_str(), Some("verify_container_startup"));
...
async fn test_readme_performance_detailed_claims() -> Result<()> {
    ...
    let _container1 = env.get_or_create_container("perf_test", || {
    ...
    // Second access (reuse)
    let reuse_start = Instant::now();
    let _container2 = env.get_or_create_container("perf_test", || {
    ...
    assert!(reuse_time < creation_time, "Container reuse should be faster than creation");
...
fn test_readme_security_policy_claims() -> Result<()> {
    ...
    for level in security_levels {
        ...
        // Test policy validation
        let validation_result = policy.validate();
        assert!(validation_result.is_ok(), "Policy should be valid for all security levels");
        
        // Test policy environment variables
        let env_vars = policy.to_env();
        assert!(env_vars.contains_key("CLEANROOM_SECURITY_LEVEL"), "Policy should generate security level env var");
    }
...
async fn test_readme_framework_self_testing_philosophy() -> Result<()> {
    ...
    assert!(test_results.total_tests >= 5, "Framework should test multiple components");
    
    // Verify that framework tests its own components
    let test_names: Vec<&str> = test_results.test_results.iter()
        .map(|r| r.name.as_str())
        .collect();
    
    assert!(test_names.contains(&"validate_framework"), "Should test framework validation");
    assert!(test_names.contains(&"test_container_lifecycle"), "Should test container lifecycle");
    assert!(test_names.contains(&"test_plugin_system"), "Should test plugin system");
...
async fn test_readme_version_history_claims() -> Result<()> {
    ...
    let test_results = run_framework_tests().await?;
    assert!(test_results.total_tests > 0, "Framework self-testing should be implemented");
    ...
    let _container = env.get_or_create_container("version_test", || {
...
impl ServicePlugin for TestPlugin {
    ...
    fn start(&self) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<ServiceHandle>> + Send + '_>> {
    ...
    fn stop(&self, _handle: ServiceHandle) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<()>> + Send + '_>> {
        Box::pin(async move {
            Ok(())
        })
    }
...
mod tests {
    ...
    async fn test_readme_example_output_claims() -> Result<()> {
        ...
        let result = env.execute_test("example_test", || {
    ...
    async fn test_readme_ci_cd_integration_claims() -> Result<()> {
        ...
        for test_result in &test_results.test_results {
            ...
            assert!(test_result.duration_ms > 0, "Test duration needed for CI/CD");
    ...
    async fn test_readme_contributing_claims() -> Result<()> {
        ...
        for component in component_tests {
            assert!(test_names.contains(&component), 
                "Framework should test component: {}", component);
        }
        ...
        assert!(success_rate >= 0.8, "Framework self-tests should have high success rate: {:.1}%", success_rate * 100.0);
