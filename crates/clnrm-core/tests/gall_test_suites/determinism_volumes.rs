...
use clnrm_core::determinism::volumes::{
    generate_container_name, generate_network_name, generate_volume_name,
};
...
fn gall_test_deterministic_volume_naming() {
    ...
    assert_eq!(
        vol1, vol2,
        "Volume names must be strictly deterministic given the same seed"
    );
    assert!(vol1.starts_with("clnrm-vol-"));
    assert_eq!(
        vol1.len(),
        "clnrm-vol-".len() + 12,
        "Should append exactly 12 chars of hash"
    );
}
...
fn gall_test_deterministic_container_naming_with_seed() {
    ...
    assert_eq!(
        c1, c2,
        "Container names must be deterministic given the same seed"
    );
...
fn gall_test_deterministic_network_naming() {
    ...
    assert_eq!(
        net1, net2,
        "Network names must be strictly deterministic given the same seed"
    );
