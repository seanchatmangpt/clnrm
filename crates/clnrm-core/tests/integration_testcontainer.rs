...
fn test_testcontainer_comprehensive() {
    ...
    assert!(
        alpine_backend.is_available(),
        "Testcontainer backend should be available"
    );
    assert!(
        alpine_backend.supports_hermetic(),
        "Testcontainer should support hermetic execution"
    );
    assert!(
        alpine_backend.supports_deterministic(),
        "Testcontainer should support deterministic execution"
    );
    assert_eq!(
        alpine_backend.name(),
        "testcontainers",
    ...
    assert!(
        basic_result.stdout.contains("Hello, World!"),
        "Output should contain expected text"
    );
    assert!(basic_result.duration_ms > 0, "Duration should be recorded");
    assert_eq!(
        basic_result.backend, "testcontainers",
        "Backend name should be correct"
    );
    assert!(
        !basic_result.concurrent,
        "Single command should not be concurrent"
    );
    ...
    assert!(
        env_result.stdout.contains("TEST_VAR=test_value"),
        "Should contain first env var"
    );
    assert!(
        env_result.stdout.contains("ANOTHER_VAR=another_value"),
        "Should contain second env var"
    );
    ...
    assert!(
        security_result.stdout.contains("High security test"),
        "Output should contain expected text"
    );
    ...
    assert!(
        ubuntu_result.stdout.contains("Ubuntu"),
        "Should show Ubuntu OS info"
    );
