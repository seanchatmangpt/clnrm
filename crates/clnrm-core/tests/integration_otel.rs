...
fn test_otel_tracing_comprehensive() {
    ...
    assert!(
        result1.stdout.contains("Hello, OTel!"),
        "Alpine output should contain expected text"
    );
    ...
    assert!(
        result2.stdout.contains("Linux"),
        "Ubuntu should show Linux kernel info"
    );
...
fn test_otel_tracing_with_env_vars() {
    ...
    assert!(
        result.stdout.contains("OTEL_SERVICE_NAME=test-service"),
        "Should contain OTel service name"
    );
    assert!(
        result
            .stdout
            .contains("OTEL_EXPORTER_OTLP_ENDPOINT=http://localhost:4317"),
        "Should contain OTLP endpoint"
    );
    assert!(
        result.stdout.contains("TEST_VAR=test-value"),
        "Should contain test variable"
    );
...
fn test_otel_collector_container_inspection() {
    ...
    match result {
        Ok(run_result) => {
            ...
            assert!(
                run_result.exit_code == 0 || run_result.exit_code == 127,
