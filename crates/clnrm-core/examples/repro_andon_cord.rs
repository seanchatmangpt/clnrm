...
async fn main() -> Result<()> {
    ...
    println!("Weaver started on port {} with PID {}", otlp_port, weaver_pid);
    ...
    let result = env.execute_test("andon_cord_test", || {
        println!("Test started, waiting 1s then killing Weaver...");
        std::thread::sleep(Duration::from_secs(1));
        
        println!("KILLING Weaver (PID {})", weaver_pid);
        let _ = std::process::Command::new("kill").arg("-9").arg(weaver_pid.to_string()).status();
