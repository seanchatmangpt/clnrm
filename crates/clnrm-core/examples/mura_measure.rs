...
use clnrm_core::services::ollama::{OllamaPlugin, OllamaConfig};
use clnrm_core::services::chaos_engine::{ChaosEnginePlugin, ChaosConfig, ChaosScenario};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let plugins: Vec<(String, Box<dyn ServicePlugin>)> = vec![
        ("OtelCollector".to_string(), Box::new(OtelCollectorPlugin::new("otel"))),
        ("Generic (Postgres)".to_string(), Box::new(GenericContainerPlugin::new("postgres", "postgres:15"))),
        ("Generic (Redis)".to_string(), Box::new(GenericContainerPlugin::new("redis", "redis:7"))),
        ("SurrealDB".to_string(), Box::new(SurrealDbPlugin::new())),
        ("Ollama".to_string(), Box::new(OllamaPlugin::new("ollama", OllamaConfig {
            endpoint: "http://localhost:11434".to_string(),
            default_model: "llama3".to_string(),
            timeout_seconds: 1,
        }))),
        ("ChaosEngine".to_string(), Box::new(ChaosEnginePlugin::with_config("chaos", ChaosConfig {
            failure_rate: 0.1,
            latency_ms: 10,
    ...
    for (display_name, plugin) in plugins {
        ...
        let target = if ms < 100.0 { "✅ PASS" } else { "❌ INC_MURA" };

        println!("| {:<17} | {:>17.2} | {:<6} | {:<15} |", display_name, ms, status, target);
    }
    ...
    if std_dev > 50.0 {
        ...
        println!("Some plugins (like ChaosEngine and potentially SurrealDB/Ollama) are 'Incapable'");
