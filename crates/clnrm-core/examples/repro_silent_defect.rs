...
use clnrm_core::backend::oci::{OciContainerConfig, OciImage, OciImageConfig, OciLayer, OciRootfs};
use std::fs;
use tar::Builder;
...
async fn main() -> anyhow::Result<()> {
    ...
    cache
        .store(image_ref, &image)
        .await
        .map_err(|e| anyhow::anyhow!("{}", e))?;

    // 4. Locate the blob on disk
    let blob_path = dirs::cache_dir()
        .unwrap()
        .join("clnrm")
        .join("oci")
        .join("layers")
    ...
    let result_dir = manager
        .extract_layer_to_cache(&retrieved_layer)
        .await
        .map_err(|e| anyhow::anyhow!("{}", e))?;
    ...
    if extracted_content.contains("CORRUPTE") {
        ...
        println!(
            "The digest was {} but the data actually contained 'CORRUPTE'.",
            retrieved_layer.digest
        );
    } else {
        println!(
            "\n[INFO] Corruption was not found in extracted content. content='{}'",
            extracted_content
        );
    }
