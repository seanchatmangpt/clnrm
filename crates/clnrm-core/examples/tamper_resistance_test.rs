...
async fn main() -> anyhow::Result<()> {
    ...
    let layer_dir = manager
        .extract_layer_to_cache(&layer)
        .await
    ...
    let rootfs_res = manager
        .mount_overlayfs(&[layer.clone()], &rootfs_mount)
        .await;
    ...
    match rootfs_res {
        Ok(rootfs) => {
            ...
            println!(
                "Content in merged mount after tampering cache: {}",
                content_after.trim()
            );
