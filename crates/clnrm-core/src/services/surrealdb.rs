...
use crate::backend::oci::{OciImageLoader, OciBundleBuilder, RunscExecutor, ImageSource};
use tracing::info;
...
impl SurrealDbPlugin {
    ...
    pub fn with_credentials(_user: &str, _pass: &str) -> Self { Self::new() }
    pub fn with_strict(self, _strict: bool) -> Self { self }
...
impl ServicePlugin for SurrealDbPlugin {
    ...
    fn start(&self) -> Result<crate::cleanroom::ServiceHandle> {
        let rt = tokio::runtime::Runtime::new().map_err(|e| CleanroomError::internal_error(e.to_string()))?;
        rt.block_on(async {
            ...
            handle.metadata.insert("container_id".to_string(), container_id.clone());
            handle.metadata.insert("bundle_path".to_string(), bundle.path.to_string_lossy().into_owned());
            handle.metadata.insert("port".to_string(), "8000".to_string());
