...
mod tests {
    ...
    fn create_span_with_times(name: &str, start: u64) -> SpanData {
        SpanData {
            name: name.to_string(),
            ...
            end_time_unix_nano: Some(start + 1000),
            kind: None,
            events: None,
    ...
    #[test]
    fn test_order_expectation_precedes() -> Result<()> {
        // Arrange
        let spans = vec![
    ...
    #[test]
    fn test_order_expectation_precedes_violation() -> Result<()> {
        // Arrange
        let spans = vec![
    ...
    #[test]
    fn test_order_expectation_follows() -> Result<()> {
        // Arrange
        let spans = vec![
            create_span_with_times("container.start", 1000),
    ...
    #[test]
    fn test_order_validator_creation() {
        // Arrange
        let spans = vec![
            create_span_with_times("span1", 1000),
            create_span_with_times("span2", 2000),
        ];
        ...
        // Assert
        assert_eq!(validator.spans_by_name.len(), 2);
    }

    #[test]
    fn test_order_validator_precedes() -> Result<()> {
        // Arrange
        let spans = vec![
            create_span_with_times("first", 1000),
            create_span_with_times("second", 2000),
        ];
        let validator = OrderValidator::new(&spans);

        // Act
        let result = validator.validate_precedes("first", "second");
    ...
    #[test]
    fn test_order_validator_follows() -> Result<()> {
        // Arrange
        let spans = vec![
            create_span_with_times("first", 1000),
            create_span_with_times("second", 2000),
        ...
        // Act
        let result = validator.validate_follows("second", "first");
