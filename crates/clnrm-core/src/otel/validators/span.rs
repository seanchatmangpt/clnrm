...
mod tests {
    ...
    fn create_test_span(name: &str, parent_id: Option<&str>, kind: Option<SpanKind>) -> SpanData {
        SpanData {
            name: name.to_string(),
    ...
    #[test]
    fn test_span_expectation_name_match() -> Result<()> {
        // Arrange
        let spans = vec![create_test_span("container.start", None, None)];
        let span_by_id = HashMap::new();
        let expectation = SpanExpectation::new("container.start");
        ...
        assert_eq!(result.spans_checked, 1);
        Ok(())
    }

    #[test]
    fn test_span_expectation_glob_pattern() -> Result<()> {
        // Arrange
        let spans = vec![
            create_test_span("container.start", None, None),
            create_test_span("container.exec", None, None),
            create_test_span("container.stop", None, None),
        ];
        let span_by_id = HashMap::new();
        let expectation = SpanExpectation::new("container.*");
        ...
        // Assert
        assert!(result.passed);
        assert_eq!(result.spans_checked, 3);
        Ok(())
    }

    #[test]
    fn test_span_expectation_no_match() -> Result<()> {
        // Arrange
        let spans = vec![create_test_span("container.start", None, None)];
        let span_by_id = HashMap::new();
        ...
        // Assert
        assert!(!result.passed);
        assert_eq!(result.spans_checked, 0);
        Ok(())
    }
    ...
    #[test]
    fn test_span_expectation_duration_validation() -> Result<()> {
        // Arrange
        let spans = vec![create_test_span("test.span", None, None)];
        let span_by_id = HashMap::new();
        let expectation = SpanExpectation::new("test.span").with_duration(Some(10), Some(200));
