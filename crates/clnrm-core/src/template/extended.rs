...
impl Function for WeightedFunction {
    fn call(&self, args: &HashMap<String, Value>) -> tera::Result<Value> {
        ...
        for pair in pairs {
            let pair_array = pair.as_array()
                .ok_or_else(|| tera::Error::msg("weighted() pairs must be arrays [value, weight]"))?;

            if pair_array.len() != 2 {
                return Err(tera::Error::msg("weighted() pairs must have exactly 2 elements"));
            }

            values.push(pair_array[0].clone());
            let weight = pair_array[1].as_f64()
...
impl Function for SampleFunction {
    fn call(&self, args: &HashMap<String, Value>) -> tera::Result<Value> {
        ...
        let k = args
            .get("k")
            .and_then(|v| v.as_u64())
            .ok_or_else(|| tera::Error::msg("sample() requires 'k' number parameter"))? as usize;
...
impl Function for FakeKindsFunction {
    fn call(&self, _args: &HashMap<String, Value>) -> tera::Result<Value> {
        let kinds = vec![
            ...
            "internet.password",
            "address.city", "address.country", "address.street.name",
            "address.street.address", "address.zip", "address.tz",
