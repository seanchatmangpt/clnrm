...
mod tests {
    ...
    async fn test_jit_container_creation_audit() {
        ...
        tracing::info!("Audit: Creating pool with min_idle = {} (JIT expected)", config.min_idle);
        let pool = ContainerPool::new(config.clone()).await.expect("Failed to create pool");
        ...
        tracing::info!("Initial Over-production Waste: {} containers", initial_waste);
        
        // ASSERT JIT: No containers should be created yet
        assert_eq!(stats.created, 0, "JIT Violation: Containers created before request");
        assert_eq!(stats.overproduction_waste(), 0, "JIT Violation: Waste detected before request");

        // Phase 2: Single Request
        tracing::info!("Audit: Acquiring 1 container");
        let handle = pool.acquire_handle().await.expect("Failed to acquire handle");
        let stats = pool.stats();
        ...
        assert_eq!(stats.overproduction_waste(), 0, "Sequential JIT should have 0 waste");
        ...
        let _handle2 = pool.acquire_handle().await.expect("Failed to acquire handle");
        let stats = pool.stats();
        ...
        assert_eq!(stats.created, 3, "Should have created 2 more containers JIT");
        ...
        tracing::info!("Over-production Waste: {} containers", stats.overproduction_waste());
        tracing::info!("Waste Percentage: {:.1}%", (stats.overproduction_waste() as f64 / stats.created as f64) * 100.0);
        tracing::info!("------------------------");
