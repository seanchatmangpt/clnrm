...
impl LiquidityIncentiveEngine {
    ...
    pub fn update_pool_with_time(&mut self, now: u64) {
        ...
        self.accumulated_reward_per_share += (token_reward * precision_factor) / self.total_staked as u128;
        self.last_update_time = now;
    }
    ...
    pub fn stake_tokens_with_time(&mut self, user_id: &str, amount: u64, now: u64) {
        ...
        if let Some(stake) = self.stakers.get(user_id) {
            ...
            let pending = ((user_amount as u128 * self.accumulated_reward_per_share) / precision_factor).saturating_sub(user_debt);
            let accumulated = self.user_accumulated_yield.entry(user_id.to_string()).or_insert(0);
        ...
        self.stakers.insert(user_id.to_string(), StakeInfo {
            amount: user_amount,
            timestamp: now,
        });
        
        self.user_reward_debt.insert(user_id.to_string(), (user_amount as u128 * self.accumulated_reward_per_share) / precision_factor);
    }
    ...
    pub fn unstake_tokens_with_time(&mut self, user_id: &str, amount: u64, now: u64) -> Result<(), String> {
        self.update_pool_with_time(now);
        ...
        *accumulated += pending as u64;
        ...
        let new_debt = (stake.amount as u128 * self.accumulated_reward_per_share) / precision_factor;
    ...
    pub fn claim_yield_with_time(&mut self, user_id: &str, now: u64) -> Result<u64, String> {
        ...
        if stake_amount > 0 {
            ...
            let current_share = (stake_amount as u128 * self.accumulated_reward_per_share) / precision_factor;
