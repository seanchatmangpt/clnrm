...
mod tests {
    ...
    fn test_span_builder_basic() {
        // Arrange & Act
        let span = SpanBuilder::new("test.span").build();

        // Assert
        assert_eq!(span.name, "test.span");
        assert_eq!(span.span_id, "span_test.span");
        assert_eq!(span.trace_id, "test_trace_123");
    ...
    fn test_span_builder_with_parent() {
        ...
        // Assert
        assert_eq!(span.parent_span_id, Some("parent_span_id".to_string()));
    }
    ...
    #[test]
    fn test_span_builder_with_status() {
        // Arrange & Act
    ...
    #[test]
    fn test_span_builder_with_error() {
        // Arrange & Act
        let span = SpanBuilder::new("test").with_error(true).build();
    ...
    #[test]
    fn test_span_builder_with_times() {
        // Arrange & Act
        let span = SpanBuilder::new("test")
        ...
        // Assert
        assert_eq!(span.start_time_unix_nano, Some(1000));
        assert_eq!(span.end_time_unix_nano, Some(2000));
    }

    #[test]
    fn test_span_builder_with_duration_ms() {
        // Arrange & Act
        let span = SpanBuilder::new("test")
            .with_start_time(1_000_000_000)
            .with_duration_ms(100)
            .build();

        // Assert
        assert_eq!(span.start_time_unix_nano, Some(1_000_000_000));
        assert_eq!(span.end_time_unix_nano, Some(1_100_000_000));
        assert_eq!(span.duration_ms(), Some(100.0));
    }
    ...
    #[test]
    fn test_span_builder_fluent_chaining() {
        // Arrange & Act
        let span = SpanBuilder::new("complex.span")
            .with_span_id("custom_span_id")
            .with_parent("parent_id")
            .with_trace_id("trace_xyz")
            .with_status("OK")
            .with_kind(SpanKind::Client)
            .with_attribute("http.method", "GET")
            .with_attribute("http.status_code", json!(200))
            .with_start_time(1000)
            .with_duration_ms(50)
            .with_event("request_sent")
            .with_event("response_received")
            .build();

        // Assert
        assert_eq!(span.name, "complex.span");
        assert_eq!(span.span_id, "custom_span_id");
        assert_eq!(span.parent_span_id, Some("parent_id".to_string()));
        assert_eq!(span.trace_id, "trace_xyz");
        assert_eq!(span.kind, Some(SpanKind::Client));
        assert_eq!(span.attributes.len(), 3); // status + 2 custom
        assert_eq!(span.duration_ms(), Some(50.0));
        assert_eq!(span.events.as_ref().map(|e| e.len()), Some(2));
    }

    #[test]
    fn test_create_span_helper() {
        // Arrange & Act
        let span = create_span("test", "span1", Some("parent1"));
    ...
    #[test]
    fn test_create_span_with_status_helper() {
        // Arrange & Act
        let span = create_span_with_status("test", "ERROR");
    ...
    #[test]
    fn test_create_span_with_error_helper() {
        // Arrange & Act
        let span = create_span_with_error("test", true);
    ...
    #[test]
    fn test_create_span_with_times_helper() {
        // Arrange & Act
        let span = create_span_with_times("test", 1000, 2000);
    ...
    #[test]
    fn test_create_span_with_events_helper() {
        // Arrange & Act
        let events = vec!["event1".to_string(), "event2".to_string()];
        let span = create_span_with_events("test", events.clone());

        // Assert
        assert_eq!(span.events, Some(events));
    }

    #[test]
    fn test_span_builder_resource_attributes() {
        // Arrange & Act
        let span = SpanBuilder::new("test")
    ...
    fn test_span_builder_event_count_attribute() {
        ...
        // Assert
        assert_eq!(
            span.attributes.get("event.count").and_then(|v| v.as_u64()),
            Some(5)
        );
    }
}
