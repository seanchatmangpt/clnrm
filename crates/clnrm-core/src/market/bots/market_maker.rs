    ...
    pub async fn run(&self) {
        ...
        while self.is_running.load(Ordering::SeqCst) {
            ...
            if action < 15 {
                ...
                let proportion = rng.random_range(0.01..0.05);
                ...
                if tokens.len() >= 2 {
                    let idx1 = rng.random_range(0..tokens.len());
                    let mut idx2 = rng.random_range(0..tokens.len());
                    while idx1 == idx2 {
                        idx2 = rng.random_range(0..tokens.len());
                    }
                    ...
                    if max_swap_size > 1.0 {
                        let amount_in = rng.random_range(1.0..max_swap_size);
