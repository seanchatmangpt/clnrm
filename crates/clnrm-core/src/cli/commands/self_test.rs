...
pub async fn run_self_tests(suite: Option<String>, report: bool) -> Result<()> {
    ...
    if report {
        crate::cli::commands::report::generate_framework_report(&results).await
...
mod tests {
    ...
    async fn test_run_self_tests_with_report() -> Result<()> {
        ...
        // Assert - Should succeed (framework self-tests should pass)
        assert!(result.is_ok(), "Framework self-tests with report should succeed: {:?}", result.err());
        
        // Verify report file was created
        let report_exists = std::fs::metadata("framework-test-report.json").is_ok();
        assert!(report_exists, "Report file should be created when report=true");
    ...
    async fn test_run_self_tests_all_valid_suites() -> Result<()> {
        ...
        for suite_name in valid_suites {
            ...
            // Assert
            assert!(result.is_ok(), "Suite '{}' should be valid and succeed", suite_name);
        }
    ...
    async fn test_run_self_tests_with_suite_and_report() -> Result<()> {
        ...
        // Assert
        assert!(result.is_ok(), "Framework self-tests with suite and report should succeed: {:?}", result.err());
    ...
    async fn test_run_self_tests_no_suite_no_report() -> Result<()> {
        ...
        // Assert
        assert!(result.is_ok(), "Framework self-tests with no suite and no report should succeed: {:?}", result.err());
        
        // Verify no report file was created
        let report_exists = std::fs::metadata("framework-test-report.json").is_ok();
        assert!(!report_exists, "Report file should not be created when report=false");
    ...
    async fn test_run_self_tests_suite_validation_case_sensitive() -> Result<()> {
        ...
        for suite_name in case_variations {
            ...
            if suite_name == "framework" {
                ...
                assert!(result.is_err(), "Uppercase '{}' should be invalid", suite_name);
            }
        }
    ...
    async fn test_run_self_tests_suite_validation_whitespace() -> Result<()> {
        ...
        for suite_name in whitespace_variations {
            ...
            assert!(result.is_err(), "Suite with whitespace '{}' should be invalid", suite_name);
        }
