...
"#;
        ...
        fs::write(&test_file1, toml_content)
            .map_err(|e| CleanroomError::internal_error("Failed to write test file 1")
                .with_source(e.to_string()))?;
        fs::write(&test_file2, toml_content)
            .map_err(|e| CleanroomError::internal_error("Failed to write test file 2")
                .with_source(e.to_string()))?;
    ...
    fn test_validate_config_directory_no_test_files() -> Result<()> {
        ...
        fs::write(&ignored_file, "ignored content")
            .map_err(|e| CleanroomError::internal_error("Failed to write ignored file")
                .with_source(e.to_string()))?;
...
"#;
    ...
    fn test_validate_single_config_file_not_found() -> Result<()> {
        ...
        // Assert
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("Test file does not exist"));
