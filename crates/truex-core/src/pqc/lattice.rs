...
#[inline]
pub fn reduce(x: i64) -> i64 {
    let x_128 = x as i128;
    let mut r = x_128 % (Q as i128);
...
impl Serialize for Poly {
    ...
    where
        S: Serializer,
    {
        self.coeffs.to_vec().serialize(serializer)
    }
}
        ...
        D: Deserializer<'de>,
    {
        ...
        for (i, &val) in v.iter().enumerate() {
            if val < min_coeff || val > max_coeff {
                return Err(DeError::custom(format!(
                    "coefficient {} at index {} is out of canonical range [{}, {}]",
                    val, i, min_coeff, max_coeff
                )));
            }
            coeffs[i] = val;
        }
...
impl Poly {
    ...
    pub fn add(&self, other: &Poly) -> Self {
        let mut r = Poly::zero();
        for i in 0..N {
            let sum_128 = (self.coeffs[i] as i128) + (other.coeffs[i] as i128);
            let reduced_128 = sum_128 % (Q as i128);
            r.coeffs[i] = reduce(reduced_128 as i64);
        }
        r
    }
    ...
    pub fn sub(&self, other: &Poly) -> Self {
        let mut r = Poly::zero();
        for i in 0..N {
            let diff_128 = (self.coeffs[i] as i128) - (other.coeffs[i] as i128);
            let reduced_128 = diff_128 % (Q as i128);
            r.coeffs[i] = reduce(reduced_128 as i64);
        }
        r
    }
    ...
    pub fn mul(&self, other: &Poly) -> Self {
        ...
        for i in 0..N {
            for j in 0..N {
                let prod = (self.coeffs[i] as i128 * other.coeffs[j] as i128) % (Q as i128);
                if i + j < N {
                    r[i + j] = (r[i + j] + prod) % (Q as i128);
                } else {
                    r[i + j - N] = (r[i + j - N] - prod) % (Q as i128);
                }
            }
        }
        let mut poly = Poly::zero();
        for i in 0..N {
            poly.coeffs[i] = reduce(r[i] as i64);
        }
        poly
    }
    ...
    pub fn high_bits(&self) -> [i64; N] {
        let mut out = [0i64; N];
        for i in 0..N {
            let mut pos_x = (self.coeffs[i] as i128) % (Q as i128);
            if pos_x < 0 {
                pos_x += Q as i128;
            }
            out[i] = (pos_x / (D as i128)) as i64;
        }
        out
    }
...
mod tests {
    ...
    #[test]
    fn test_poly_deserialization_bounds() {
        // Valid JSON with correct length and values within [-Q/2, Q/2]
        let mut valid_coeffs = vec![0i64; N];
        valid_coeffs[0] = Q / 2;
        valid_coeffs[1] = -Q / 2;
        let valid_json = serde_json::to_string(&valid_coeffs).unwrap();
        let poly: Result<Poly, _> = serde_json::from_str(&valid_json);
        assert!(poly.is_ok(), "Should successfully deserialize valid coefficients");

        // Invalid JSON: one coefficient exceeds Q/2
        let mut invalid_coeffs_high = valid_coeffs.clone();
        invalid_coeffs_high[2] = Q / 2 + 1;
        let invalid_json_high = serde_json::to_string(&invalid_coeffs_high).unwrap();
        let poly_high: Result<Poly, _> = serde_json::from_str(&invalid_json_high);
        assert!(poly_high.is_err(), "Should fail to deserialize due to out of bounds coefficient (too high)");

        // Invalid JSON: one coefficient is below -Q/2
        let mut invalid_coeffs_low = valid_coeffs.clone();
        invalid_coeffs_low[2] = -Q / 2 - 1;
        let invalid_json_low = serde_json::to_string(&invalid_coeffs_low).unwrap();
        let poly_low: Result<Poly, _> = serde_json::from_str(&invalid_json_low);
        assert!(poly_low.is_err(), "Should fail to deserialize due to out of bounds coefficient (too low)");

        // Invalid JSON: incorrect length
        let short_coeffs = vec![0i64; N - 1];
        let short_json = serde_json::to_string(&short_coeffs).unwrap();
        let poly_short: Result<Poly, _> = serde_json::from_str(&short_json);
        assert!(poly_short.is_err(), "Should fail to deserialize due to invalid length");
    }

    #[test]
    fn test_modular_calculation_overflow_handling() {
        // Construct polynomials manually with maximum values
        let mut p1 = Poly::zero();
        p1.coeffs[0] = i64::MAX;
        let mut p2 = Poly::zero();
        p2.coeffs[0] = i64::MAX;

        // Verify that addition does not panic
        let res_add = p1.add(&p2);
        // It should calculate sum safely using modular arithmetic
        let expected_val = reduce(((i64::MAX as i128 + i64::MAX as i128) % Q as i128) as i64);
        assert_eq!(res_add.coeffs[0], expected_val);

        // Verify that subtraction does not panic
        let res_sub = p1.sub(&p2);
        assert_eq!(res_sub.coeffs[0], 0);

        // Verify that multiplication does not panic
        let res_mul = p1.mul(&p2);
        let expected_mul_val = reduce(((i64::MAX as i128 * i64::MAX as i128) % Q as i128) as i64);
        assert_eq!(res_mul.coeffs[0], expected_mul_val);
    }
}
