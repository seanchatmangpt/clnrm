...
fn test_cli_builder_introspection() -> Result<()> {
    let cli = Cli::new()
        ...
        .noun(noun!(
            "test",
            "Test command",
            [verb!("action", "Test action", |_args: &VerbArgs| {
                Ok(())
            }),]
        ));
...
fn test_macro_expansion() -> Result<()> {
    ...
    let test_noun = noun!(
        "test-noun",
        "Test noun",
        [verb!("test-verb", "Test verb", |_args: &VerbArgs| {
            Ok(())
        }),]
    );
...
fn test_error_types() -> Result<()> {
    ...
    assert!(cmd_error
        .to_string()
        .contains("Command 'missing-command' not found"));
    assert!(verb_error
        .to_string()
        .contains("Verb 'missing-verb' not found for noun 'services'"));
