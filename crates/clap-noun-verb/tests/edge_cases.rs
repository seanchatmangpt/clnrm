...
use clap::Arg;
use clap_noun_verb::{
    noun, verb, Cli, VerbArgs, Result, NounVerbError, CommandRegistry
};
...
fn test_duplicate_verb_names_validation() -> Result<()> {
    ...
    let _registry = registry.register_noun(noun!("test", "Test", [
...
fn test_pathbuf_extraction() -> Result<()> {
    let cli = Cli::new()
        .name("path-test")
        .noun(noun!("file", "File operations", [
...
fn test_multiple_values_extraction() -> Result<()> {
    let cli = Cli::new()
        .name("multi-test")
        .noun(noun!("items", "Item operations", [
...
fn test_auto_validation_enabled() -> Result<()> {
    let registry = CommandRegistry::new()
        ...
        .auto_validate(true)
        .register_noun(noun!("valid", "Valid noun", [
            verb!("action", "Action", |_args: &VerbArgs| { Ok(()) }),
        ]));
...
fn test_error_propagation() -> Result<()> {
    ...
    let result = cli.run_with_args(vec!["error-test".to_string(), "test".to_string(), "fail".to_string()]);
