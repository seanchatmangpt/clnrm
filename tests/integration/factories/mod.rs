...
mod tests {
    ...
    fn test_command_builder() {
        ...
        assert_eq!(cmd.working_dir, Some("/home".to_string()));
    }
    ...
    fn test_random_data_generator() {
        let mut gen = RandomDataGenerator::new();

        let s1 = gen.random_string(10);
        let s2 = gen.random_string(10);
        assert_eq!(s1.len(), 10);
        assert_eq!(s2.len(), 10);
        assert_ne!(s1, s2); // Should be different

        let port = gen.random_port();
        assert!(port >= 10000 && port < 60000);

        let duration = gen.random_duration_ms();
        assert!(duration >= 10 && duration < 1000);
    }
}
