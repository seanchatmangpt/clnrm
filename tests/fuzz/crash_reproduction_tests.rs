...
mod property_tests {
    ...
    /// Property test: Any string input to parse_toml_config should not panic
    #[test]
    fn property_parse_toml_never_panics() {
        let test_inputs = vec![
            "",
            "invalid",
            "[]",
    ...
    /// Property test: Error formatting should never panic
    #[test]
    fn property_error_display_never_panics() {
        ...
        let messages = vec!["", "test", "a".repeat(1000), "\0", "🚀"];

        for kind in error_kinds {
            for msg in &messages {
                let error = CleanroomError::new(kind.clone(), msg);
                let _ = format!("{}", error);
                let _ = format!("{:?}", error);
            }
        }
