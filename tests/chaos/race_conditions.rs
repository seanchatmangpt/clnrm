...
#[cfg(test)]
mod advanced_race_tests {
    use super::*;

    /// Test memory ordering race conditions
    #[tokio::test]
    async fn test_memory_ordering_races() -> Result<()> {
        ...
        println!("  Value read: {} (expected: 42)", value);

        assert_eq!(value, 42,
            "Acquire-Release ordering should prevent reordering");

        Ok(())
    }

    /// Test work-stealing race conditions
    #[tokio::test]
    async fn test_work_stealing_races() -> Result<()> {
        use tokio::sync::mpsc;
        ...
        drop(work_rx); // Drop original receiver
        ...
        println!("  Completed: {} (expected: 100)", total_completed);
