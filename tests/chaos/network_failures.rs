...
mod integration_tests {
    ...
    /// Test network resilience under combined chaos
    #[tokio::test]
    async fn test_combined_network_chaos() -> Result<()> {
        let engine = ChaosEnginePlugin::new("combined_chaos")
            .with_latency(300)
            .with_failure_rate(0.4);

        // Run multiple chaos scenarios concurrently
        let scenarios = vec![
        ...
        let metrics = engine.get_metrics().await;
        println!("Combined chaos metrics:");
