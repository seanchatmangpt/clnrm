...
#[cfg(test)]
mod crash_resilience_tests {
    use super::*;

    /// Test system stability under continuous crashes
    #[tokio::test]
    async fn test_continuous_crash_recovery() -> Result<()> {
        let engine = ChaosEnginePlugin::new("continuous_crash")
        ...
        while start.elapsed() < duration {
            if engine.inject_failure("worker_process").await? {
                crash_count += 1;
                println!("Crash #{}: Recovering...", crash_count);
        ...
        println!("Continuous crash test:");
        println!("  Crashes: {}", crash_count);
        println!("  Recoveries: {}", recovery_count);
        println!("  Duration: {:?}", start.elapsed());

        assert_eq!(crash_count, recovery_count,
            "All crashes should have corresponding recoveries");
